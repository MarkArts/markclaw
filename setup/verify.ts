...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.markclaw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.markclaw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active markclaw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
  ...
  // Slack: check .env for both tokens
  if (
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'markclaw', 'mount-allowlist.json'),
    )
  ) {
