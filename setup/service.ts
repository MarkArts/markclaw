...
<dict>
    ...
    <string>${projectRoot}/logs/markclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/markclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.markclaw');
  } catch {
...
/**
 * Kill any orphaned markclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned markclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/markclaw.service';
    ...
    unitPath = path.join(unitDir, 'markclaw.service');
...
StandardOutput=append:${projectRoot}/logs/markclaw.log
StandardError=append:${projectRoot}/logs/markclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned markclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable markclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start markclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active markclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-markclaw.sh');
  const pidFile = path.join(projectRoot, 'markclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-markclaw.sh — Start MarkClaw without systemd',
    ...
    '    echo "Stopping existing MarkClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting MarkClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/markclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/markclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "MarkClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/markclaw.log"`,
  ];
