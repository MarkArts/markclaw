...
This skill adds Slack support to MarkClaw using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.markclaw/state.yaml`. If `slack` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
If `.markclaw/` directory doesn't exist yet:
...
- Records the application in `.markclaw/state.yaml`
...
```bash
npm run build
launchctl kickstart -k gui/$(id -u)/com.markclaw
```
...
>
> The JID format for MarkClaw is: `slack:C0123456789`
...
4. Service is running: `launchctl list | grep markclaw`
...
6. Restart: `launchctl kickstart -k gui/$(id -u)/com.markclaw`
...
- **Multi-channel** — Can run alongside other channels if added (auto-enabled by credentials)
