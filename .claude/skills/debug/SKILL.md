...
|-----|----------|---------|
| **Main app logs** | `logs/markclaw.log` | Host-side routing, container spawning |
| **Main app errors** | `logs/markclaw.error.log` | Host-side errors |
...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash markclaw-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash markclaw-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash markclaw-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
# Run test query
echo '{"prompt":"What is 2+2?","groupFolder":"test","chatJid":"slack:C_TEST_001","isMain":false}' | \
  docker run -i \
  -v $(pwd)/data/env:/workspace/env-dir:ro \
  -v $(pwd)/groups/test:/workspace/group \
  -v $(pwd)/data/ipc:/workspace/ipc \
  markclaw-agent:latest
```
...
```bash
docker run --rm -it --entrypoint /bin/bash markclaw-agent:latest
```
...
# Also clear the session ID from MarkClaw's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/markclaw.log | tail -5
...
```bash
echo "=== Checking MarkClaw Container Setup ==="
...
SESSIONS=$(grep "Session initialized" logs/markclaw.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
