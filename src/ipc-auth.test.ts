...
beforeEach(() => {
  ...
  groups = {
    'slack:C_MAIN_001': MAIN_GROUP,
    'slack:C_OTHER_002': OTHER_GROUP,
    'slack:C_THIRD_003': THIRD_GROUP,
  };

  // Populate DB as well
  setRegisteredGroup('slack:C_MAIN_001', MAIN_GROUP);
  setRegisteredGroup('slack:C_OTHER_002', OTHER_GROUP);
  setRegisteredGroup('slack:C_THIRD_003', THIRD_GROUP);
...
describe('register_group authorization', () => {
  it('non-main group cannot register a group', async () => {
    ...
    expect(groups['slack:C_NEW_004']).toBeUndefined();
  });
...
describe('IPC message authorization', () => {
  ...
  it('main group can send to any group', () => {
    expect(
      isMessageAuthorized('slack_main', true, 'slack:C_OTHER_002', groups),
    ).toBe(true);
    expect(
      isMessageAuthorized('slack_main', true, 'slack:C_THIRD_003', groups),
    ).toBe(true);
  });

  it('non-main group can send to its own chat', () => {
    expect(
      isMessageAuthorized('other-group', false, 'slack:C_OTHER_002', groups),
    ).toBe(true);
  });

  it('non-main group cannot send to another groups chat', () => {
    expect(isMessageAuthorized('other-group', false, 'slack:C_MAIN_001', groups)).toBe(
      false,
    );
    expect(
      isMessageAuthorized('other-group', false, 'slack:C_THIRD_003', groups),
    ).toBe(false);
  });

  it('non-main group cannot send to unregistered JID', () => {
    expect(
      isMessageAuthorized('other-group', false, 'slack:C_UNKNOWN_005', groups),
    ).toBe(false);
  });
  ...
  it('main group can send to unregistered JID', () => {
    ...
    expect(
      isMessageAuthorized('slack_main', true, 'slack:C_UNKNOWN_005', groups),
    ).toBe(true);
  });
});
...
describe('register_group success', () => {
  it('main group can register a new group', async () => {
    ...
    const group = getRegisteredGroup('slack:C_NEW_004');
  ...
  it('register_group rejects request with missing fields', async () => {
    ...
    expect(getRegisteredGroup('slack:C_PARTIAL_006')).toBeUndefined();
  });
});
