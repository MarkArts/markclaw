...
let storedDeps: SchedulerDependencies | null = null;
  ...
  const loop = async () => {
    try {
      ...
      for (const task of dueTasks) {
        ...
        // Advance next_run immediately so the next poll doesn't pick it up again.
        // The actual next_run is recalculated in updateTaskAfterRun() after completion.
        let provisionalNext: string | null = null;
        if (currentTask.schedule_type === 'cron') {
          const interval = CronExpressionParser.parse(
            currentTask.schedule_value,
            { tz: TIMEZONE },
          );
          provisionalNext = interval.next().toISOString();
        } else if (currentTask.schedule_type === 'interval') {
          const ms = parseInt(currentTask.schedule_value, 10);
          provisionalNext = new Date(Date.now() + ms).toISOString();
        }
        updateTask(currentTask.id, { next_run: provisionalNext });
