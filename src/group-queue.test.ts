...
describe('GroupQueue', () => {
  ...
  it('only runs one container per group at a time', async () => {
    ...
    queue.enqueueMessageCheck('slack:C0000000001');
    queue.enqueueMessageCheck('slack:C0000000001');
  ...
  it('respects global concurrency limit', async () => {
    ...
    queue.enqueueMessageCheck('slack:C0000000001');
    queue.enqueueMessageCheck('slack:C0000000002');
    queue.enqueueMessageCheck('slack:C0000000003');
  ...
  it('drains tasks before messages for same group', async () => {
    ...
    queue.enqueueTask('slack:C0000000001', 'task-1', taskFn);
    queue.enqueueMessageCheck('slack:C0000000001');
  ...
  it('drains waiting groups when active slots free up', async () => {
    ...
    expect(processed).toEqual(['slack:C0000000001', 'slack:C0000000002']);

    // Free up a slot
    completionCallbacks[0]();
    await vi.advanceTimersByTimeAsync(10);

    expect(processed).toContain('slack:C0000000003');
  });
  ...
  it('preempts idle container when task is enqueued', async () => {
    ...
    queue.notifyIdle('slack:C0000000001');
  ...
  it('sendMessage resets idleWaiting so a subsequent task enqueue does not preempt', async () => {
    ...
    queue.sendMessage('slack:C0000000001', 'hello');
  ...
  it('sendMessage returns false for task containers so user messages queue up', async () => {
    ...
    const result = queue.sendMessage('slack:C0000000001', 'hello');
